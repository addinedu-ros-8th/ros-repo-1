...
class STTNode(Node):
    ...
    def action_function_listening(self, listen_type):
        self.get_logger().info(" 음성 인식 시작... (모드: {listen_type})")
        ...
        try:
            for transcript in transcript_generator:
                if transcript:
                    ...
                    if listen_type == "walk":
                        self.publish_string(transcript, self.__pub_to_handler)

                    elif listen_type == "talk":
